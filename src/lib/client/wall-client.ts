...
export class WallClient {
    async getById(ids: { id: number, owner_id: number }[]): Promise<WallGetResponse> {
        ...
        return await baseClient.get<WallGetResponse>("wall.getById?" + url.toString())
    }

    async wallGet(owner_id: number, count = 50, offset = 0) {
        const url = new URLSearchParams()
    ...
    async like(ownerId: number, id: number) {
        ...
        return await baseClient.get<void>("likes.add?" + url.toString())
    }
}